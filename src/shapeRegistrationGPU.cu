...
__global__ void imageMomentKernel(PixelCoords *d_pImg, int d_lenForeground,
                                  float *d_mmt, int d_mmtDegree) {
  int x = threadIdx.x + blockDim.x * blockIdx.x;
  ...
  if (x < d_lenForeground) {
    ...
    for (int q = 0; q < d_mmtDegree; q++) {
      for (int p = 0; p < d_mmtDegree; p++) {
        ...
        d_mmt[mmtIndex * d_lenForeground + x] =
            pow(d_pImg[x].x, p + 1) * pow(d_pImg[x].y, q + 1);
      }
    }
  }
}
...
void imageMomentGPU(PixelCoords *h_pImg, int h_lenForeground, float *h_mmt,
  ...
  dim3 grid = dim3((h_lenForeground + block.x - 1) / block.x, 1, 1);
  ...
  imageMomentKernel<<<grid, block>>>(d_pImg, h_lenForeground, d_mmt,
                                     h_mmtDegree);
...
void cutMarginsGPU(float *h_imgIn, int h_w, int h_h, float *&h_resizedImg,
  ...
  cutMarginsGPUKernel<<<grid, block>>>(d_imgIn, d_resizedImg, h_w, h_resizedW,
                                       h_resizedH, h_margins);
  CUDA_CHECK;
...
void centerOfMassGPU(float *h_imgIn, int h_w, int h_h, float &h_xCentCoord,
  ...
  centerOfMassKernel<<<grid, block, 3 * nThreads * sizeof(float)>>>(
      d_imgIn, h_w, h_h, d_xCentCoord, d_yCentCoord, d_numOfForegroundPixelOut);
  CUDA_CHECK;
...
__global__ void pCoordsNormalisationKernel(
    int d_w, int d_h, PixelCoords *d_pCoords, float d_xCentCoord,
...
void pCoordsNormalisationGPU(int h_w, int h_h, PixelCoords *h_pCoords,
                             ...
                             float &h_normXFactor, float &h_normYFactor) {
  dim3 block = dim3(16, 8, 1);
  dim3 grid =
      dim3((h_w + block.x - 1) / block.x, (h_h + block.y - 1) / block.y, 1);
  ...
  pCoordsNormalisationKernel<<<grid, block>>>(h_w, h_h, d_pCoords, h_xCentCoord,
                                              h_yCentCoord, d_normXFactor,
                                              d_normYFactor);
...
__global__ void qCoordsNormalisationKernel(
    int d_w, int d_h, QuadCoords *d_qCoords, float d_xCentCoord,
    float d_yCentCoord, float *d_normXFactor, float *d_normYFactor) {
  int index;
...
void qCoordsNormalisationGPU(int h_w, int h_h, QuadCoords *h_qCoords,
  ...
  qCoordsNormalisationKernel<<<grid, block>>>(h_w, h_h, d_qCoords, h_xCentCoord,
                                              h_yCentCoord, d_normXFactor,
                                              d_normYFactor);
...
__global__ void pCoordsDenormalisationKernel(int d_w, int d_h,
                                             PixelCoords *d_pCoords,
                                             float d_xCentCoord,
                                             float d_yCentCoord) {
  int index;
...
void pCoordsDenormalisationGPU(int h_w, int h_h, PixelCoords *h_pCoords,
  ...
  pCoordsDenormalisationKernel<<<grid, block>>>(h_w, h_h, d_pCoords,
                                                h_xCentCoord, h_yCentCoord);
...
__global__ void pTPSGPUKernel(int d_lenForeground, PixelCoords *d_pCoords,
...
void pTPSGPU(int h_lenForeground, PixelCoords *h_pCoords, TPSParams h_tpsParams,
  ...
  pTPSGPUKernel<<<grid, block>>>(h_lenForeground, d_pCoords, h_tpsParams,
                                 h_cDim);
  CUDA_CHECK;
  ...
  if (x < d_lenForeground) {
    for (int qIndex = 0; qIndex < 4; qIndex++) {
      ...
      for (int k = 0; k < dimSize; k++) {
        ...
        Q = radialApproxKernel(d_qCoords[x].x[qIndex], d_qCoords[x].y[qIndex],
                               d_tpsParams.ctrlP[k],
                               d_tpsParams.ctrlP[k + dimSize]);
...
void qfTPSGPU(int h_lenForeground, QuadCoords *h_qCoords,
              TPSParams &h_tpsParams, int h_cDim) {
  dim3 block = dim3(128, 1, 1);
  dim3 grid = dim3((h_lenForeground + block.x - 1) / block.x, 1, 1);
...
__global__ void jacobianTransGPUKernel(int d_lenForeground, float *d_jacobi,
                                       PixelCoords *d_pCoords,
  ...
  if (x < d_lenForeground) {
    ...
    for (int k = 0; k < K; k++) {
      squareOfNorm = (d_tpsParams.ctrlP[k] - d_pCoords[x].x) *
                         (d_tpsParams.ctrlP[k] - d_pCoords[x].x) +
                     (d_tpsParams.ctrlP[k + K] - d_pCoords[x].y) *
                         (d_tpsParams.ctrlP[k + K] - d_pCoords[x].y);
...
void jacobianTransGPU(int h_lenForeground, float *h_jacobi,
                      PixelCoords *h_pCoords, TPSParams h_tpsParams,
                      int h_c_dim) {
  dim3 block = dim3(128, 1, 1);
  dim3 grid = dim3((h_lenForeground + block.x - 1) / block.x, 1, 1);
  ...
  jacobianTransGPUKernel<<<grid, block>>>(h_lenForeground, d_jacobi, d_pCoords,
                                          h_tpsParams, h_c_dim);
  CUDA_CHECK;
...
void transferGPU(float *h_imgIn, PixelCoords *h_pCoords, QuadCoords *h_qCoords,
  ...
  transferKernel<<<grid, block>>>(d_imgIn, d_pCoords, d_qCoords, h_t_w, h_t_h,
                                  h_o_w, h_o_h, d_imgOut);
...
void lmminObjectiveWrapperGPU(const double *par, const int m_dat,
  ...
  objectiveFunctionGPU(observationImg, templateImg, ro_w, ro_h, Normalisation,
                       tpsParams, qTemplate, pTemplate, pObservation, rt_w,
                       rt_h, t_sx, t_sy, o_sx, o_sy, residual);
...
void objectiveFunctionGPU(float *observationImg, float *templateImg, int ro_w,
  ...
  getCoordForeground(observationImg, pObservation, ro_w, ro_h, pfObservation);
  getCoordForeground(templateImg, pTemplate, rt_w, rt_h, pfTemplate);
  ...
  dim3 block = dim3(128, 1, 1);
  dim3 grid = dim3((t_lenForeground + block.x - 1) / block.x, 1, 1);
  ...
  pTPSGPUKernel<<<grid, block>>>(t_lenForeground, d_pCoords, tpsParams,
                                 DIM_C_REF);
  CUDA_CHECK;
  ...
  imageMomentKernel<<<grid, block>>>(d_pCoords, t_lenForeground, dt_mmt,
                                     momentDeg);
